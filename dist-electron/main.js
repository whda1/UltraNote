...
function saveWithDialog() {
  return ipcMain.handle("saveWithDialog", async (event, { fileContent, fileName }) => {
    ...
    const { filePath } = await dialog.showSaveDialog(window, {
      ...
      // Suggest a default path and name
    });
    if (filePath) {
      try {
...
function saveWithoutDialog() {
  return ipcMain.handle("saveWithoutDialog", async (event, { fileContent, fileName, filePath }) => {
    if (filePath) {
      try {
        const fullFilePath = filePath + path$1.sep + fileName;
        fs$1.writeFileSync(fullFilePath, fileContent);
        return fullFilePath;
      } catch (err) {
        console.error("Failed to save file:", err);
        return null;
